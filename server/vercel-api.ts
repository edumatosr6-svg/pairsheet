...
async function snapshot(user: User) { const row = await readState(); return { state: JSON.parse(row.data), revision: row.revision, user }; }
...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    if (['/api/auth/login', '/api/auth/setup'].includes(path) && req.method === 'POST') {
      const input = await body(req); const parsed = path.endsWith('/setup') ? credentials.parse(input) : accessPassword.parse(input);
      ...
      if (path.endsWith('/setup')) {
        ...
        const name = z.string().trim().min(2).max(100).parse(input.name); const password = await passwordHash(setup.password);
        const user = await transaction(async tx => {
          ...
          const user: User = { id: randomUUID(), email: setup.email, name, role: 'admin' };
      ...
      const user = await one<User & { password_hash: string }>("SELECT * FROM users WHERE role='admin' LIMIT 1");
      const valid = await passwordMatches(parsed.password, user?.password_hash ?? '00000000000000000000000000000000:' + '00'.repeat(64));
      if (!user || !valid) return json(res, 401, { error: 'Senha incorreta.' });
    ...
    if (path === '/api/access/password') {
      ...
      await transaction(async tx => {
        ...
        await tx.execute({ sql: 'INSERT INTO security_audit(actor,message,at) VALUES (?,?,?)', args: [user.id, 'Senha de acesso alterada', new Date().toISOString()] });
